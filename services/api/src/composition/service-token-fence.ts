 ...
 * tenant + an active payment agent and mints a propose/approve-capable token),
 * so it carries the same two boot invariants as demo-provision-fence.ts:
 *
 ...
 *   - Scopes: the minted JWT carries READ + PROPOSE + APPROVE only — never
 *     payment_intent:execute, audit:admin, or policy:write.
  ...
  if (input.nodeEnv === "production" && input.testnetAttested !== "true") {
    throw new Error(
      "BRAIN_SERVICE_TOKEN_ENABLED=true on NODE_ENV=production but " +
        ...
        "route mints propose/approve-capable tokens for per-user tenants; enabling " +
