...
| `services/agents/` _(not in pnpm)_ | Python package            | Python container: reconciliation agent + stubs                                 | `services/agents/brain_agents/`                                                                        | Medium     |
| `contracts/` _(not in pnpm)_       | Foundry project           | 4 Solidity contracts, Base Sepolia deployed                                    | `contracts/src/*.sol`                                                                                  | High       |
...
| `services/raw`       | `services/raw/migrations/`       | 7          | Artifacts, parsed, plaid*items, sources (new), force_rls. \*\*Warning: two files share prefix `0004*`** (`0004_force_rls.sql`+`0004_raw_plaid_items_rls.sql`). Migration ordering conflict risk. |
...
_Exact queue names unverified. Derived from worker registration patterns. Confirm in `queues/bullmq-queues.md`._
...
_Evidence basis: `git log --since=2026-05-24` (74 files, +4408/−117), Explore agent reads of per-workspace `package.json` and entrypoints, grep of `services/api/src/main.ts`, `BLOCKERS.md`, `HARDENING-SUMMARY.md`, `CLAUDE.md`, `docker-compose.yml`, `pnpm-workspace.yaml`._
