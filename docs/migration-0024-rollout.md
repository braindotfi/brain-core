...
- **Tenants with an existing user or agent row** get a real admin whose `member.id` equals
  the earliest user id — i.e. that user's JWT principal. For these tenants `/v1/members`
  flips 403 → 200 and member CRUD works under the current prod code. If you are testing
...
`members`) in one short transaction — no locks on `audit_events`.
