...
| Balance reservation lifecycle  | `services/ledger/src/reservations-facade.ts`, `services/execution/src/outbox/` | The approved to dispatching handoff creates a ledger reservation atomically with outbox enqueue; the outbox row carries `reservation_id`; settlement consumes it and deterministic failure releases it. x402/escrow are exempt because their spend is enforced on-chain.                      |
...
| Runtime DB role matrix                 | `services/api/src/composition/runtime-db-roles.ts`           | production pools connect as the expected least-privilege roles and lack representative forbidden privileges.                                                                                           |
