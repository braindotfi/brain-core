...
function readyFixture(overrides = {}) {
  const files = {
    ...
    "shared/src/gate/evidence-validator.ts":
      "const RULES = { pay_invoice: [], pay_obligation: [] };",
    ...
    "services/execution/src/payment-intents/routes.ts":
      'app.get("/payment-intents/:id/replay-investigation", h);',
    "services/agent-router/src/agent-api.test.ts":
      'it("halt-category stops all agents", () => {});',
...
test("fails when the agent has no rail allowlist in promotion-config.ts", () => {
  const dir = readyFixture({
    "services/agent-router/src/promotion-config.ts":
      "export const LIVE_AGENTS = { liveAgents: {} };",
  });
  const { rows } = runReadinessChecks({
    repo: dir,
    agent: "savings",
    attest: ["onchain_behavior_hash", "session_key_grants"],
  });
