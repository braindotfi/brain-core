...
const POLICY = JSON.parse(
  readFileSync(join(ROOT, "infra/minio/brain-worker-artifacts-v1.json"), "utf8"),
);
const PREPARE = join(ROOT, "scripts/ops/prepare-minio-worker-env.sh");
...
test("worker receives only the rendered scoped S3 credential", () => {
  const worker = serviceBlock("worker");
  const common = COMPOSE.match(/^x-brain-common-env:[\s\S]*?(?=^services:)/m)?.[0] ?? "";

  assert.match(worker, /path: \.env\.worker\.prod/);
  assert.match(worker, /required: true/);
...
test("managed worker setup applies the exact approved policy", () => {
  const setup = serviceBlock("minio-worker-setup");
  assert.match(setup, /path: \.env\.minio-worker/);
  assert.match(setup, /brain-worker-artifacts-v1\.json/);
  assert.match(setup, /mc admin user add local/);
  assert.match(setup, /mc admin policy attach local brain-worker-artifacts-v1/);

  const statements = POLICY.Statement;
  assert.deepEqual(statements[0], {
    Sid: "ListArtifactVersions",
    Effect: "Allow",
    Action: ["s3:ListBucketVersions"],
    Resource: ["arn:aws:s3:::brain-artifacts"],
  });
  assert.deepEqual(statements[1], {
    Sid: "WorkerArtifactLifecycle",
    Effect: "Allow",
    Action: [
      "s3:GetObject",
      "s3:PutObject",
...
test("worker env renderer removes root and source credential names", () => {
  const root = mkdtempSync(join(tmpdir(), "brain-worker-env-"));
  try {
    const source = join(root, ".env.prod");
    const credentials = join(root, ".env.minio-worker");
    const output = join(root, ".env.worker.prod");
    writeFileSync(
      source,
      [
    ...
    assert.match(result, /worker_credential_status=created_or_repaired/);
...
test("worker env renderer refuses an unexpected managed access key", () => {
  const root = mkdtempSync(join(tmpdir(), "brain-worker-env-"));
  try {
    const source = join(root, ".env.prod");
    const credentials = join(root, ".env.minio-worker");
    ...
    writeFileSync(
      credentials,
      "MINIO_WORKER_ACCESS_KEY_ID=unexpected\nMINIO_WORKER_SECRET_ACCESS_KEY=hidden\n",
      { mode: 0o600 },
    );
    ...
    assert.match(result.stdout, /MINIO_WORKER_ACCESS_KEY_ID_status=unexpected_value/);
...
test("deploy workflows prepare and reconcile the scoped identity before worker recreate", () => {
  for (const path of [".github/workflows/main.yml", ".github/workflows/promote-prod.yml"]) {
    ...
    assert.ok(recreateAt > reconcileAt, `${path} must reconcile before worker recreation`);
    assert.match(workflow, /infra\/minio\/brain-worker-artifacts-v1\.json/);
    assert.match(workflow, /scripts\/ops\/prepare-minio-worker-env\.sh/);
  }
});

test("production verifier pins SHA, identity, root absence, and lifecycle", () => {
  const workflow = readFileSync(join(ROOT, ".github/workflows/ops-minio-worker-scope.yml"), "utf8");
  assert.match(workflow, /environment: production/);
  assert.match(workflow, /Exact deployed commit SHA to verify/);
  assert.match(workflow, /worker_effective_identity=brain-worker/);
  assert.match(workflow, /test -z "\$\{MINIO_ROOT_USER\+x\}"/);
  assert.match(workflow, /worker_policy=brain-worker-artifacts-v1/);
  assert.match(workflow, /verify-minio-worker-lifecycle\.mjs/);
