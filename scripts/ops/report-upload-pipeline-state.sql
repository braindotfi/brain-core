...
SELECT cp.id, cp.name, cp.type, cp.canonical_counterparty_id, cp.source_ids, cp.evidence_ids
  FROM ledger_counterparties cp
 WHERE cp.owner_id = :'tenant_id'
 ORDER BY cp.created_at ASC, cp.id ASC;

\echo 'ledger_counterparty_type_constraint'
SELECT pg_get_constraintdef(con.oid) AS definition
  FROM pg_constraint con
 WHERE con.conrelid = 'ledger_counterparties'::regclass
   AND con.conname = 'ledger_counterparties_type_check';
