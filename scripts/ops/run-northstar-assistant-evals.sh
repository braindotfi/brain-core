...
readonly script_path="/tmp/run-northstar-assistant-evals.mjs"
...
if [[ "$(printf '%s\n' "$bootstrap_external_ref" | wc -l | tr -d ' ')" != "1" ]]; then
  echo "northstar_assistant_evals_failed=bootstrap_admin_identity_not_unique" >&2
  exit 1
fi

docker cp /tmp/run-northstar-assistant-evals.mjs "brain-prod-api:$script_path"
