...
if [[ "${TARGET:-}" != "production" ]]; then
  echo "phase A diagnostic must target production"
  exit 1
fi
...
if grep -Eiq '(pg_dump|pg_basebackup|backup|restic|borg|azcopy|mc[[:space:]]+mirror|anchor|verifier)' \
  "$REPORT_DIR/host-schedules.txt"; then
  echo "overlapping host schedule found"
  grep -Ei '(pg_dump|pg_basebackup|backup|restic|borg|azcopy|mc[[:space:]]+mirror|anchor|verifier)' \
    "$REPORT_DIR/host-schedules.txt"
  exit 1
fi
