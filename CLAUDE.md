...
- **Payment rails — real implementations, live wiring pending**: `bank_ach` (`rails/ach-plaid.ts`, `AchPlaidRail` — two-step Plaid Transfer, async webhook settlement) and `onchain_base` (`rails/onchain-base.ts`, `OnchainBaseRail` — `BrainSmartAccount.executeViaSessionKey`, nonce-threaded, KMS-signed) now have real implementations against injected clients (`PlaidTransferClient` / `OnchainExecutor`), fully unit-tested with mocks. The **live SDK construction** (the `plaid` SDK, viem + `@azure/keyvault-keys`) and the integration verification (Plaid sandbox round-trip, anvil deploy-and-dispatch) are the remaining follow-up — see `services/execution/README.md`. The `*StubRail` classes in `rails/stubs.ts` (used by `defaultRails()`) are retained for dev/test and `erp_writeback`; they **fail closed under `NODE_ENV=production`** (`defaultRails()` and each dispatch throw rather than fake-settle), mirroring the boot fence in `services/api/src/main.ts`. Production registration of the real rails in `RailRegistry` happens at boot when their clients are configured.
- **Reconciliation matchers**: all 7 are concrete implementations (`services/ledger/src/reconciliation/{statement-balance,card-charge,invoice-payment,transaction-receipt,wallet-transfer,payroll-bank-debit,subscription-charge}.ts`), registered in `ReconciliationService`, and covered by unit + fast-check property tests (in the ledger coverage gate).
