...
- **Python agents**: all four MVP agents are implemented under `services/agents/brain_agents/`: `reconciliation/`, `payment/`, `anomaly/`, and `plaid_extractor/`. Each owns a FastAPI router (`/run/<agent>`); they share `BrainApiClient` for `/v1/execution/propose` and `/v1/raw/ingest` calls. `plaid_extractor` is deterministic (no LLM); the other three reason via OpenAI. mypy strict + pytest 80% coverage gates apply.
