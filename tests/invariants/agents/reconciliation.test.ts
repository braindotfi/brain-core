/**
 * Adversarial invariants — reconciliation agent.
 */
...
describe("reconciliation agent — action injection", () => {
  it("injected action not in event_action_map resolves to missing_action", async () => {
    const def = internalAgentDefinitions.reconciliation!;
    const r = await resolver.resolve({
      definition: def,
      actions: ["propose_match"],
      context: { [REQUESTED_ACTION_KEY]: "delete_transaction" },
    });
    expect(r.status).toBe("missing_action");
  });
});

describe("reconciliation agent — scope restriction", () => {
  it("reconciliation_review not granted → agent not selected", async () => {
    const router = new AgentRouter({
      catalog: () => [internalAgentDefinitions.reconciliation!],
