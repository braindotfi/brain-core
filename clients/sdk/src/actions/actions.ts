...
export class ActionsModule {
  ...
  public async create(opts: CreateActionInput): Promise<Action> {
    const body: Record<string, unknown> = {
      ...
      ...(opts.to !== undefined
        ? { to: { counterparty_id: opts.to.counterpartyId } }
        : {}),
  ...
  public async list(
    ...
    return this.http.get<{ data: Action[]; next_cursor: string | null }>(
      "/actions",
      {
        query: {
          tenantId: opts.tenantId,
  ...
  public async approve(
    ...
    const body: Record<string, unknown> = {
      ...
      ...(opts.approverRole !== undefined
        ? { approver_role: opts.approverRole }
        : {}),
