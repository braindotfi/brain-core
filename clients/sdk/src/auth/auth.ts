...
export class AuthModule {
  ...
  ): Promise<SiwxChallenge> {
    return this.http.post<SiwxChallenge>(
      "/auth/siwx/challenge",
      opts.agentAddress !== undefined
   ...
   */
  public async signInWithSIWX(
    opts: SignInWithSIWXOptions,
  ): Promise<SignInWithSIWXResult> {
    const body: Record<string, unknown> = {
      message: opts.message,
      signature: opts.signature,
      ...(opts.sessionId !== undefined ? { session_id: opts.sessionId } : {}),
    };
    const result = await this.http.post<SignInWithSIWXResult>(
      "/auth/siwx",
      body,
    );
