...
export class AuditModule {
   ...
   */
  public async get(eventId: string): Promise<{ event: AuditEvent; inclusion_proof: AuditProof | null }> {
    return this.http.get(
      `/audit/event/${encodeURIComponent(eventId)}`,
    );
  }
  ...
  public async proof(eventId: string): Promise<AuditProof> {
    return this.http.get<AuditProof>(
      `/audit/event/${encodeURIComponent(eventId)}/proof`,
    );
  }
  ...
  public async exportStatus(jobId: string): Promise<AuditExportStatus> {
    return this.http.get<AuditExportStatus>(
      `/audit/export/${encodeURIComponent(jobId)}`,
    );
  }
