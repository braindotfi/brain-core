...
| Code     | Meaning                                                                |
| -------- | --------------------------------------------------------------------- |
| `-32001` | JWT invalid/expired (`auth_token_*`) — only when surfaced in-dispatch |
...
After revocation, the on-chain `revokedAt` becomes non-zero, so the scope read returns null and all calls are rejected pre-dispatch with an HTTP `401` `agent_not_registered_onchain` envelope — within the 60-second cache window.
